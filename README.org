...
;; Git backup (requires git in PATH)
(org-todoist-git-log) ;; View git commit history for sync buffer
(org-todoist-git-show-diff) ;; Show diff for a specific commit
(org-todoist-git-restore) ;; Restore sync buffer from a previous commit
...
Org-todoist automatically tracks sync buffer history using git (when enabled), providing complete protection against data loss. Each sync creates automatic commits before and after the operation, allowing you to:

- View the complete history of all sync operations
- Review changes made during any sync
- Restore the sync buffer from any previous point in time
- **Rollback local changes** if needed by restoring from a pre-sync commit

#+begin_src emacs-lisp
;; Git backup is enabled by default. To disable:
(setq org-todoist-use-git-backup nil)

;; View git commit history:
(org-todoist-git-log)                ;; Show recent commits

;; Show diff for a specific commit:
(org-todoist-git-show-diff)          ;; Select and view commit changes

;; Restore from a previous commit:
(org-todoist-git-restore)            ;; Restore sync buffer from history
#+end_src

Git must be available in your PATH for this feature to work. The git repository is automatically initialized in ~org-todoist-storage-dir~ on the first sync when git backup is enabled.

*Note:* Git backup tracks:
- Your org-todoist file (todoist.org) with all local changes
- The sync buffer file (used for diffing changes)
- The sync token

It does not track debug files like last-request.json and last-response.json. A copy of your todoist.org file is stored in the storage directory for git tracking, ensuring you can always rollback to any previous state before or after a sync operation.
...
- [X] Backups (git-based backup system tracks sync buffer history)
- [ ] Markdown support
- [ ] Activity log
- [ ] Ramble
...
- ~org-todoist-use-git-backup~ - Enable git-based backup of sync buffer history (default: t)
  - Requires git to be available in PATH
  - Automatically creates commits before and after each sync
  - Provides complete history and ability to restore from any point
- ~org-todoist-git-commit-message-format~ - Format string for git commit messages (default: "Auto-commit: %s at %s")
