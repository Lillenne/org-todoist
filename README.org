...
- CRUD[fn:1] tasks
  ...
  - Assignee[fn:3]
  - Reminders (absolute and relative time)
...
;; Reminder management
(org-todoist-add-absolute-reminder) ;; Add reminder at specific date/time
(org-todoist-add-relative-reminder) ;; Add reminder X minutes before due
(org-todoist-remove-reminder) ;; Remove reminder from task
...
** Managing reminders
#+begin_src emacs-lisp
(org-todoist-add-absolute-reminder) ;; Add a reminder at a specific date/time
(org-todoist-add-relative-reminder) ;; Add a reminder X minutes before due date
(org-todoist-remove-reminder) ;; Remove the reminder from a task
#+end_src

Reminders are stored in the task's property drawer under the ~reminders~ property using one of two formats:
- Absolute: ~absolute|YYYY-MM-DDTHH:MM:SS~ - Reminder at a specific date and time
- Relative: ~relative|MINUTES~ - Reminder X minutes before the task's due date (requires task to have a scheduled/due date)

Example task with absolute reminder:
#+begin_example
**** TODO Submit report
SCHEDULED: <2025-01-15 Wed 17:00>
:PROPERTIES:
:reminders: absolute|2025-01-15T16:00:00
:END:
#+end_example

Example task with relative reminder (30 minutes before due):
#+begin_example
**** TODO Team meeting
SCHEDULED: <2025-01-20 Mon 14:00>
:PROPERTIES:
:reminders: relative|30
:END:
#+end_example
...
- [-] Notifications/reminders
  ...
  - [X] Absolute time reminders
  - [X] Relative time reminders (minutes before due)
  - [ ] Location-based reminders
  - [ ] Multiple reminders per task
  - [ ] Reminder editing and deletion (currently adds new reminders)
- [-] Labels
