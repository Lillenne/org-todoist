...
*Note:* Git backup tracks the sync buffer file (used for diffing changes) and sync token. It does not track debug files like last-request.json and last-response.json.
